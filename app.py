...
with app.app_context():
    ...
    default_pw = generate_password_hash('AdminPass1')
    cur.execute(
...
def book():
    ...
    # validere antal gæster
    if size < 1 or size > 10:
        flash('Antal gæster skal være mellem 1 - 10','error')
    ...
    # confirmation email
    msg = Message("Din reservation", recipients=[email])
    msg.body = f"Hej {name}, dit bord er reserveret d. {start.strftime('%d-%m-%Y %H:%M')} til {size} gæster."
    mail.send(msg)
...
def admin_booking_create():
    if request.method=='POST':
        ...
        msg = Message(
          "Dit Crisp Reservation er Bekræftet",
          recipients=[email]
        )
        msg.body = (
          f"Hej {name},\n\n"
          f"dit bord er reserveret d. {start:%d-%m-%Y %H:%M} "
          f"til {size} gæster.\n\n"
          "– Crisp Burger"
        )
        mail.send(msg)
...
def admin_booking_edit(id):
    ...
    if request.method=='POST':
        ...
        msg = Message(
          "Dit Crisp Reservation er blevet ændret",
          recipients=[row['email']]
        )
        msg.body = (
          f"Hej {row['name']},\n\n"
          f"Dit reservation er blevet ændret til d. "
          f"{new_dt:%d-%m-%Y %H:%M} til {new_size} gæster.\n\n"
          "– Crisp Burger"
        )
        mail.send(msg)
...
def admin_booking_delete(id):
    ...
    msg = Message(
      "Dit Crisp Reservation er blevet annulleret",
      recipients=[row['email']]
    )
    msg.body = (
      f"Hej {row['name']},\n\n"
      f"Din reservation for {human} "
      "er blevet annuleret.\n\n"
      "– Crisp Burger"
    )
    mail.send(msg)
